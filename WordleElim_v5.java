				...
				case 'R':	//recommends word based on rating system
					...
					if(word.size()<3) {
						...
						for(int n=0; n<dictionary.size(); n++) {
							for(int i=0; i<dictionary.get(n).length(); i++) {
								rate[n]=rate[n]+alpha[(int)(dictionary.get(n).charAt(i))-65][i];
							}
						}
						int index=0;
						for(int n=0; n<dictionary.size(); n++) {
						...
						for(int n=0; n<possible.size(); n++) {
							for(int i=0; i<possible.get(n).length(); i++) {
								rate[n]=rate[n]+alpha[(int)(possible.get(n).charAt(i))-65][i];
							}
						}
						int index=0;
						for(int n=0; n<possible.size(); n++) {
